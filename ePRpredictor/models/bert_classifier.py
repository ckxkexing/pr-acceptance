...
class CodeBERTClassifer(torch.nn.Module):
    def __init__(
        self, args, text_tokenizer_len=None, diff_tokenizer_len=None, n_classes=2
    ):
        ...
        self.transformer = RobertaModel.from_pretrained(
            args.desc_backbone, config=config
        )
        self.transformer2 = RobertaModel.from_pretrained(
            args.diff_backbone, config=config
        )
    ...
    def forward(
        self, ids, mask, token_type_ids, features, true_label, gen_feature=False
    ):
