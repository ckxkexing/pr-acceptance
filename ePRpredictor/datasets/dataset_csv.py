...
def standardiseNumericalFeats(X_train, X_test, X_val=None):
    ...
    numerical_cols = [
        "number_of_pr_in_this_proj_before_pr",
        "commit_add_line",
        "commit_delete_line",
        "commit_total_line",
        "commit_file_change",
...
class csvDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, args, X, max_length=500, vocab=None, vocab2index=None):
        ...
        if "BERT" in args.model:
            ...
            self.diff_tokenizer.add_special_tokens(
                {
                    "additional_special_tokens": [
                        "</->",
                        "</+>",
    ...
    def char_tokenizer(self, input):
        input = [
            self.vocabulary.index(i) + 1 for i in list(input) if i in self.vocabulary
        ]
    ...
    def _hide_features(self, features, columns, cols_to_hide):
        for i, v in enumerate(columns.isin(cols_to_hide)):
            if v:
                features[i] = 0
    ...
    def __getitem__(self, index):
        ...
        hide = self.args.hide
        if hide in self.hide_options:
            self._hide_features(features, columns, self.hide_options[hide])
        ...
        #     features = [(i-j) / k for i, j, k in zip(features, self.proj_feature_mean[project_name],
        #                                              self.proj_feature_sd[project_name])]
