...
from .utils import read_json_data, write_csv_data, mkdir, read_csv_data
...
def find_out_commit_type(pulls_commit_file_dir, project_name, csv_file_path):
    ...
    for info in tqdm(all_data):
        mp[info['commit_url'].split('/')[-1]] = {'files':info['commit_detail']['files']}
    ...
    csv_file = os.path.join(csv_file_path, f'{project_name}_commit_contain_test_or_doc.csv')
