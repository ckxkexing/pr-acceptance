...
path = (
    "/data1/chenkexing/star_50/clone_star_50_projects_only_commits/django/django/.git"
)
repo = Repo(path)
...
for commit in commits:
    # print("Committed by %s on %s with sha %s" % (commit.committer.name,
    #                                              time.strftime("%a, %d %b %Y %H:%M",
    #                                                            time.localtime(commit.committed_date)), commit.hexsha))
    cnt += 1
    # print("#")
